...
/* -------------------------------------------------
   ...
   Accept camelCase AND kebab-case
-------------------------------------------------- */
const API_MAP = {
  campaigns: "campaigns",
  sessions: "sessions",
...
async function fetchWithFallback(primaryUrl, fallbackUrl, options) {
  ...
  // If it looks like a route-shape mismatch, try the fallback.
...
export const cmApi = {
  ...
  async update(type, id, record) {
    ...
    // 🔥 FIX: server already returns camelCase — do NOT normalize again
  ...
  /* -------------------------------------------------
     DELETE alias (Option 2)
     Allows UI to call cmApi.delete(...)
  -------------------------------------------------- */
