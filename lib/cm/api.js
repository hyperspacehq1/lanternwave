...
export const cmApi = {
  ...
  async update(type, id, record) {
    ...
    // ✅ NORMALIZE — always return a single record object
    if (data?.row) return normalizeRow(data.row);
    if (data?.record) return normalizeRow(data.record);

    // Common per-entity wrappers
    if (data?.campaign) return normalizeRow(data.campaign);
    if (data?.session) return normalizeRow(data.session);
    if (data?.event) return normalizeRow(data.event);
    if (data?.encounter) return normalizeRow(data.encounter);
    if (data?.location) return normalizeRow(data.location);
    if (data?.npc) return normalizeRow(data.npc);
    if (data?.item) return normalizeRow(data.item);
    if (data?.player) return normalizeRow(data.player);

    // Rows array wrapper
    if (Array.isArray(data?.rows)) {
      return normalizeRow(data.rows[0] ?? null);
    }
