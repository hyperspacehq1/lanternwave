...
export async function applyTemplate(tenantId, campaignId, templateKey) {
  ...
  // 2. Import join tables with mapped IDs - NOW INCLUDING campaign_id!
  const joinTables = [
  ...
  for (const { name: tableName, entityTypes } of joinTables) {
    ...
    for (const join of joins) {
      ...
      const fkColumns = Object.keys(join).filter(k => 
        k.endsWith('_id') && !['id', 'tenant_id', 'campaign_id'].includes(k)
      );
      
      // Start with base columns including campaign_id
      const values = [newId, tenantId, campaignId];
      const columns = ['id', 'tenant_id', 'campaign_id'];
      ...
      for (const fkCol of fkColumns) {
        ...
        if (!newFkId) {
          ...
          console.warn(`[CampaignImporter] Skipping this join record`);
          skipRecord = true;
          break;
        }
      ...
      if (skipRecord) {
        continue; // Skip this join record
      }

      if (columns.length > 3) { // Only insert if we have FK columns (more than id, tenant_id, campaign_id)
        const placeholders = values.map((_, i) => `$${i + 1}`).join(', ');
